...
struct Readme: ParsableCommand {
    @Argument(help: "The template file will be used to parse and replace content.")
    var template: String
    ...
    func run() throws {
        ...
        let newContent = try String(contentsOf: URL(fileURLWithPath: template), encoding: .utf8).replacingOccurrences(of: "{latest_posts}", with: formattedPosts)
